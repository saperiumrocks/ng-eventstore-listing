...
class NgEventstoreListingComponent {
    ...
    _initializeRequests() {
        ...
        this._exportPlaybackListSubscription = this._exportPlaybackListSubject
            ...
            .subscribe((result) => {
            const /** @type {?} */ csv = new Blob([result.data], { type: 'text/csv' });
            const /** @type {?} */ moment = moment_;
            const /** @type {?} */ now = moment();
            const /** @type {?} */ fileName = `${now.format('YYYY_MM_DD_HH_mm_ss')}_${this.csvFileName}.csv`;
            saveAs(csv, fileName);
        });
    }
    /**
    ...
    _initSubscriptions() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            self._dataList.forEach((row) => __awaiter(this, void 0, void 0, function* () {
                ...
                this._subscriptionTokens.push(yield self.playbackService.registerForPlayback(self.itemSubscriptionConfiguration.playbackScriptName, self, query, self.stateFunctions, row.get('revision') + 1, self._playbackList));
            }));
            // List subscription
            this._subscriptionTokens.push(yield self.playbackService.registerForPlayback(self.listSubscriptionConfiguration.playbackScriptName, self, self.listSubscriptionConfiguration.query, self.stateFunctions, 0, self._playbackList));
        });
    }
    /**
     * @return {?}
